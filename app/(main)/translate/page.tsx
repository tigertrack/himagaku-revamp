...
import { upsertTranslatedVocab } from '@/functions/dexie'
...
const Page = () => {
    ...
    const getTranslation = async () => {
        try {
            ...
            // Save each vocab kanji to the local DB, incrementing frequency
            await Promise.all(
                (result.vocabs as vocab[]).map((v) => upsertTranslatedVocab(v.kanji))
            );
        } catch (error) {
            console.log(error)
        } finally {
